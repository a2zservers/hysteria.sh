...
apt install -y screen squid stunnel4 gnutls-bin python
...
cd /etc || exit
wget 'https://api.memorykey.net/paymemory/installer/socksws' -O /etc/socks.py
wget 'https://api.memorykey.net/paymemory/installer/sockssl' -O /etc/sockssl.py
dos2unix /etc/socks.py
dos2unix /etc/sockssl.py
...
PREMIUM="username='$username' AND auth_vpn=md5('$password') AND status='active' AND premium_duration > 0"
VIP="username='$username' AND auth_vpn=md5('$password') AND status='active' AND vip_duration > 0"

Query="SELECT username FROM users WHERE $PREMIUM OR $VIP"
...
iptables -t nat -A POSTROUTING -s 10.10.0.0/22 -o "$server_interface" -j MASQUERADE
iptables -t nat -A POSTROUTING -s 10.10.0.0/22 -o "$server_interface" -j SNAT --to-source "$server_ip"
...
screen -dmS sockssl python /etc/sockssl.py 8443
...
* * * * * /bin/bash /etc/.hysteria >/dev/null 2>&1"; } | crontab -
